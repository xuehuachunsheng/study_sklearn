...
每一份分配给子线程，子线程会在迭代的时候计算部分样本的中心，并返回每个簇的样本个数，这是为了方便
进行全局样本中心的更新。在主线程拿到所有节点的中心和对应的样本个数，就可以在在主线程上做全局中心的更新。
全局变量
    样本数量 -- n_samples
    样本维度 -- n_dim
    全局中心 -- centers
    子线程个数 -- num_nodes
    子线程将要计算的样本下标范围（或者在文件中的行号范围） -- sample_idxx_range
    聚类结果 -- cluster_results_all
    所有子线程所计算的中心 -- c_centerss
    所有子线程所计算的每个簇的样本个数 -- n_samples_each_cluster_node

测试结果：
    运行20次聚类的平均时间开销：4.8s. 
    就时间复杂度上来讲，这个和单线程的没啥区别，但是存在线程的分配开销。
'''
...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        while abs(np.sum((t_centers - centers)**2)) > 1e-2:
            centers = t_centers
            cX = np.empty(num_nodes, dtype=object)
            for i in range(num_nodes):
                cX[i] = ((X[sample_idxx_range[i, 0]:sample_idxx_range[i, 1], :], i), None)
            # Parallel computing process
            reqs = tp.makeRequests(map, cX, callback=reduce)
            myPool = tp.ThreadPool(num_nodes)
            for req in reqs:
                myPool.putRequest(req)
            myPool.wait() # Wait other sub-threads
